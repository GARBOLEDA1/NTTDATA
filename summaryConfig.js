...
function generateXrayJUnitXML(data, fileName = '', fileContent = '', options) {
    ...
    forEach(data.metrics, function (metricName, metric) {
        ...
        forEach(metric.thresholds, function (thresholdName, threshold) {
            if (threshold.ok) {
                cases.push(
                    '<testcase name="'+ `${__ENV.PROJECT_KEY} ` + escapeHTML(metricName) + ' - ' + escapeHTML(thresholdName) + '">' +
                    ...
                    '<property name="test_summary" value="'+ `${__ENV.PROJECT_KEY} ` + escapeHTML(metricName) + ' - ' + escapeHTML(thresholdName) + '"/>' +
                    '</properties>' +
                    '</testcase>'
                )
            } else {
